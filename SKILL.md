---
name: refine
description: Refine an OpenClaw skill or agent by collaborating with the agent to self-diagnose runtime issues and iterate with mandatory review rounds. Use when the user invokes `/refine`, reports a specific symptom from an OpenClaw agent's output (jargon, missing personalization, wrong tone, regressed quality, broken plan-drafting), wants to compare 2+ design paths/architectures via a frozen-rubric ladder (R1-R4), or needs dual-scorer validation before a production gate. Not for generic code, build, lint, or dependency debugging unrelated to OpenClaw skills.
...
Look at the user's most recent message in this conversation — the invocation that triggered this skill. Take the text after `/refine` (or, if invoked without an explicit slash, the symptom they described that auto-triggered this skill) as the user's request:

- If it is the literal token `init` (with no other text, or `init` followed by whitespace and arguments), enter the **`/refine init` flow** in the Setup section above. Do not start a refinement session.
- Otherwise, treat it as the symptom or topic for **Phase 1 (Diagnose)**. Begin the cross-check pre-flight, then dispatch to the agent.
- If there is no text after `/refine` (empty invocation), ask the user one line: "What skill or symptom should we refine? (Or run `/refine init` to set up a config.)" Do not start a refinement session blind.
